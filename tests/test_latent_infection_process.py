...
def test_LatentInfectionProcess_multi_subpop():
    """
    Tests that LatentInfectionProcess can sample with multiple sub-populations.
    This catches issues like ARProcess being called without a required name
    argument inside the multi-subpop branch of sample().
    """
    i0_first_obs_n_rv = DeterministicVariable("i0_first_obs_n_rv", 1e-6)
    log_r_mu_intercept_rv = DeterministicVariable("log_r_mu_intercept", 0.08)
    eta_sd_rv = DeterministicVariable("eta_sd", 0)
    ...
    with numpyro.handlers.seed(rng_seed=42):
        latent_infections, latent_infections_subpop = my_latent_infection_model(
            n_days_post_init=n_days_post_init
        )

    assert latent_infections.size == n_initialization_points + n_days_post_init
    assert latent_infections_subpop.shape == (
        n_initialization_points + n_days_post_init,
        len(pop_fraction),
    )
